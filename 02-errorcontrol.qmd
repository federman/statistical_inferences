...
## Positive predictive value{#sec-ppv}
...
For example, in @fig-animatep you see the *p*-value plotted on the y-axis (from 0 to 1) and the sample size plotted on the x-axis (from 0 to 200). For this simulation, the true effect size was d = 0, meaning there is no true effect. We can thus only observe true negatives or false positives. As the sample size increases, the *p*-value slowly moves up and down (remember from the Chapter 1 on [*p*-values](#sec-pvalues) that when there is no true effect, *p*-values are uniformly distributed). In @fig-animatep the *p*-value drops below the grey line (indicating an alpha level 0.05) after collecting 83 participants in each condition, only to drift back upwards to larger *p*-values. From this figure, it becomes clear that the more often we look at the data, and the larger the total sample size, the higher the probability that one of the analyses will yield a p < $\alpha$. If resources are infinite, the Type 1 error rate will be 1, and a researcher can always find a significant result through optional stopping.  

```{r fig-animatep, echo=FALSE}
#| fig-cap: "Simulated *p*-values for each additional observation when the null is true."

if (knitr::is_latex_output()) {
  knitr::include_graphics("images/animatep.png")
}

if (knitr::is_html_output()){
  knitr::include_graphics("images/animatep.gif")
}
...
# file.copy(filenames[n], "images/animatep.png")
# 
# # make gif for html
...
```{r optionalstopsim, eval=FALSE, cache = TRUE}
...
Start by running the simulation without changing any values, so simulating 100 participants in each condition, looking 5 times at your data, with an alpha of 0.05. Note the 50.000 simulations take a while! You should see something similar to @fig-optionalstopfig below (which is based on 500.000 simulations to make the pattern very clear).

```{r fig-optionalstopfig, eval=TRUE, echo=FALSE, cache=TRUE}
#| fig-cap: "Simulation of 500000 studies performing 5 interim analyses at an alpha level of 5%."

N <- 100 # total datapoints (per condition)
looks <- 5 # set number of looks at the data
nsims <- 500000 # number of simulated studies
alphalevel <- 0.05 # set alphalevel
...
Even though in *theory* the Type 1 and Type 2 error rate should be justified by the researcher (as Neyman and Pearson [-@neyman_problem_1933] write above), in *practice* researchers tend to imitate others. The default use of an alpha level of 0.05 can already be found in the work by Gosset on the *t*-distribution [@cowles_origins_1982; @kennedy-shaffer_05_2019], who believed that a difference of two standard errors (a z-score of 2) was sufficiently rare. The default use of 80% power (or a 20% Type 2 error rate) is similarly based on personal preferences by @cohen_statistical_1988, who writes: 
...
```{r justifyalpha1, warnings = FALSE, echo=FALSE, fig.show='hide'}
suppressWarnings({
  res1 <- JustifyAlpha::optimal_alpha(power_function = "pwr::pwr.t.test(d = 0.5, n = 80, sig.level = x, type = 'two.sample', alternative = 'two.sided')$power", error = "balance", costT1T2 = 4, printplot = FALSE)

  res2 <- JustifyAlpha::optimal_alpha(power_function = "pwr::pwr.t.test(d = 0.5, n = 64, sig.level = x, type = 'two.sample', alternative = 'two.sided')$power", error = "minimize", costT1T2 = 4, printplot = TRUE)
})
```

For example, imagine a researcher plans to collect 64 participants per condition to detect a d = 0.5 effect, and weighs the cost of Type 1 errors 4 times as much as Type 2 errors. This is exactly the scenario Cohen (1988) described, and with 64 participants per condition the relative weight of Type 1 and Type 2 errors yields a 5% Type 1 error rate and a 20% Type 2 error rate. Now imagine that this researcher realizes they have the resources to collect 80 observations instead of just 64. With an interest in an effect size of d = 0.5, the relative weight of Type 1 and Type 2 errors of 4 (a as suggested by Cohen) would be satisfied if they were to set the alpha level to `r round(res1$alpha, 3)`, as the Type 2 error rate would be `r round(res1$beta, 3)`. Alternatively, the researcher might have decided to collect 64 observations, but rather than balance the error rates, instead set the alpha level such that the weighted combined error rate is minimized, which is achieved when the alpha level is set to `r round(res2$alpha, 3)`, as visualized in @fig-minerror (for further information, see @maier_justify_2022).

```{r fig-minerror, echo=FALSE, warnings = FALSE, fig.cap='Weighted combined error rate, minimized at alpha = 0.037.'}
suppressWarnings({
  res2$plot
})
```
...
d_dist <- suppressWarnings(dt(x * sqrt(N / 2), df = (N * 2) - 2, ncp = ncp) * sqrt(N / 2)) # calculate distribution of d based on t-distribution
# Set max Y for graph
y_max <- max(d_dist) + 1
# create plot
par(bg = backgroundcolor)
d <- round(d, 2)
plot(-10, xlim = c(low_x, high_x), ylim = c(0, y_max), xlab = substitute(paste("Cohen's d")), ylab = "Density", main = substitute(paste("Distribution of Cohen's d, n = 50")))
...
crit_d <- abs(qt(alpha_level / 2, (N * 2) - 2)) / sqrt(N / 2)
...
opts_p <- c(
   ...
   answer = "The proportion of false positives for all experiments we have performed is only 5% when $H_0$ is true for all 200 studies.",
...
```{r, echo = FALSE, results = 'asis'}
opts_p <- c(
   answer = "Increase the % of a-priori true hypotheses",
...
```{r, echo = FALSE, results = 'asis'}
opts_p <- c(
   answer = "When the probability of examining a true hypothesis is low, combined with either low power or substantial bias (e.g., p-hacking).",
...
opts_p <- c(
   ...
   answer = "regardless of the power, the proportion of false positives for all experiments we have performed is 5%, and the PPV is 0% (all significant results are false positives)."
)
cat(longmcq(opts_p))
```
...
```{r, echo = FALSE, results = 'asis'}
opts_p <- c(
   "If the *p*-value drops below 0.05, it stays below 0.05.",
   "The *p*-value randomly moves between 0 and 1, and will every now and then end up below 0.05.",
   answer = "The *p*-value often drops below 0.05 well before 200 participants in each condition. In around 50% of the simulations, this already happens at N = 100.", 
   "The *p*-value will typically move below 0.05 and stay there for some time, but given a large enough sample, it will always move back up to *p* > 0.05."
)
cat(longmcq(opts_p))
```
...
opts_p <- c(
   "The *p*-value randomly moves between 0 and 1, and will every now and then end up below 0.05.",
   answer = "The *p*-values drop below and stay below 0.05 much earlier than when the true effect size is 0.3.",
   "*p*-values are meaningful when effect sizes are large (e.g., d = 0.8), but meaningless when effect sizes are small (e.g., d = 0.3).", 
   "When you examine a large effect, whenever a *p*-value drops below 0.05, it will always stay below 0.05 as the sample size increases."
)
cat(longmcq(opts_p))
```

**Q4**: Looking at @fig-optionalstopfig, which statement is true?


```{r, echo = FALSE, results = 'asis'}
opts_p <- c(
   "Optional stopping does not impact the Type 1 error rate.",
   answer = "Optional stopping inflates the Type 1 error rate. We can see this in the first five bars (*p*-values between 0.00 and 0.05), which are substantially higher than the horizontal line.",
...
```{r, echo = FALSE, results = 'asis'}
opts_p <- c(
   "At each look, the Type 1 error rate is higher than the alpha level (0.05). When using optional stopping (and reporting only the lowest *p*-value), the Type 1 error rate is higher than 0.05.",
   "At each look, the Type 1 error rate is approximately equal to the alpha level (0.05). When using optional stopping (and reporting only the lowest *p*-value), the alpha level also approximately equals the alpha level (0.05).",
   answer = "At each look, the Type 1 error rate is approximately equal to the alpha level (0.05). When using optional stopping, the Type 1 error rate is higher than the alpha level (0.05)."
)
cat(longmcq(opts_p))
```
...
opts_p <- c(
   ...
   answer = "The Type 1 error rate at each look is approximately 0.016, and the overall alpha level is approximately 0.05.", 
...
```{r, echo = FALSE, results = 'asis'}
opts_p <- c(
   "**Without** Pocock’s boundary, **small** *p*-values (e.g., *p* = 0.01) are **more** likely than slightly **higher** *p*-values (*p* = 0.04). **With** Pocock’s boundary, **small** *p*-values (e.g., *p* = 0.01) are **also more** likely than slightly **higher** *p*-values (*p* = 0.04).",
   "**Without** Pocock’s boundary, **small** *p*-values (e.g., *p* = 0.01) are **more** likely than slightly **higher** *p*-values (*p* = 0.04). **With** Pocock’s boundary, **small** *p*-values (e.g., *p* = 0.01) are **less** likely than slightly **higher** *p*-values (*p* = 0.04).",
   answer = "**Without** Pocock’s boundary, **small** *p*-values (e.g., *p* = 0.01) are **less** likely than slightly **higher** *p*-values (*p* = 0.04). **With** Pocock’s boundary, **small** *p*-values (e.g., *p* = 0.01) are **more** likely than slightly **higher** *p*-values (*p* = 0.04).", 
   "**Without** Pocock’s boundary, **small** *p*-values (e.g., *p* = 0.01) are **less** likely than slightly **higher** *p*-values (*p* = 0.04). **With** Pocock’s boundary, **small** *p*-values (e.g., *p* = 0.01) are **also less** likely than slightly **higher** *p*-values (*p* = 0.04)."
)
cat(longmcq(opts_p))
```
